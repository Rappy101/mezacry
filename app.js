...
const relistenLabel = document.getElementById("relistenLabel");
...
async function startRelistenPlayback() {
  const ctx = getAudioCtx();
  if (ctx.state === "suspended") {
    try { await ctx.resume(); } catch (e) {}
  }
  if (ctx.state !== "running") {
    showToast("Playback couldn't start — try tapping relisten again.");
    return;
  }
  ...
  relistenLabel.textContent = "Stop relisten";
...
function stopRelistenPlayback() {
  ...
  if (relistenPlaying) {
    ...
    relistenLabel.textContent = "Relisten mode";
